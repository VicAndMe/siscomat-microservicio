...
class PeticionPrevisualizacion(BaseModel):
    nombre_curso: str = Field(..., max_length=152, example="Introducción a R (Prueba)")
    nombre_participante: str = Field(..., max_length=152, example="Nombre Apellido de Prueba")
    plantilla_base64: str = Field(..., description="PDF base codificado en Base64")


class PeticionGeneracionIndividual(BaseModel):
    nombre_curso: str = Field(..., max_length=152, example="Introducción a R")
    nombre_participante: str = Field(..., max_length=152, example="Víctor Andrés Horta Félix")
